...
export class PanierComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.panierService.panier$.subscribe(
      (panier: Commande | undefined) => (this.panier = panier)
    );
    ...
    if (panierFromLocalStorage !== null) {
      try {
        ...
        console.error(
          'Error while parsing local storage data (must be panier type undefined): '
        );
      }
    }
  }
  ...
  getTotal(): number {
    let total = 0;
    if (this.panier) {
      this.panier.estconstitueedes.forEach(
        (estconstitueede) =>
          (total +=
            estconstitueede.quantite * estconstitueede.presentation.prix)
      );
    }
    return total;
  }
    ...
    try {
      let reponse = await lastValueFrom(
        this.httpClient.get<Presentation[]>(
          'http://localhost:8080/commande/valider/' +
            user +
            '/' +
            this.panier!.id
        )
      ...
      if (reponse.length != 0) {
        ...
        reponse.forEach((presentation) => {
          text += ' - ' + presentation.codecis.nom + ' \n';
        });
        ...
        if (confirm(text)) {
          ...
          await lastValueFrom(
            this.httpClient.get<Boolean>(
              'http://localhost:8080/commande/validerforce/' +
                user +
                '/' +
                this.panier!.id
            )
